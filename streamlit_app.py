...
<style>
  ...
  .stTabs [data-baseweb="tab-list"] {{
      background: {BORD}; border-radius: 10px; padding: 4px; gap: 4px;
  }}
  ...
  [data-testid="stMetric"] {{
      ...
      border-left: 3px solid {BLUE}; border: 1px solid {BORD};
  ...
  .sec {{ font-size: 1.2rem; font-weight: 700; color: {TEXT};
          border-bottom: 2px solid {BORD}; padding-bottom: 6px; margin: 8px 0 16px 0; }}
...
with col_h2:
    delta_pct = (RECUPERADO_TOTAL / META_TOTAL * 100) - 100
...
with tab1:
    ...
    st.info(f"**Brecha: ${gap/1e6:.2f}M** — las **{PROMESAS_CAIDAS} promesas caídas** explican ~{PROMESAS_CAIDAS/PROMESAS_GEN*100:.0f}% del desvío. "
            f"Principales factores: sin liquidez ({df_motivos.iloc[0]['Casos']} casos) y contacto perdido ({df_motivos.iloc[1]['Casos']} sin rellamada).")
...
with tab2:
    ...
    with col_r:
        ...
        colors = [GREEN, AMBER, SLATE, RED, "#9333ea"]
    ...
    st.warning("**WhatsApp** lidera en conversión (30%). **SMS** al 15% — se recomienda A/B test con mensajes personalizados por segmento.")
    ...
    with sub4:
        ...
        with col_l:
            ...
            fig = px.bar(df_estado.sort_values("Recuperado"), x="Recuperado", y="Estado", orientation="h",
                         color="Recuperado", color_continuous_scale=["#bfdbfe","#3b82f6","#1d4ed8"],
                         text_auto=".2s")
            fig.update_traces(textfont_color=TEXT)
        ...
        with col_r:
            ...
            fig2 = make_subplots(specs=[[{"secondary_y": True}]])
            fig2.add_trace(go.Bar(x=df_segmento["Segmento"], y=df_segmento["Recuperado"],
                                  name="Recuperado $", marker_color=BLUE, opacity=0.8), secondary_y=False)
            fig2.add_trace(go.Scatter(x=df_segmento["Segmento"], y=df_segmento["Cumplimiento %"],
                                      name="% Cumplimiento", mode="lines+markers",
                                      line=dict(color=AMBER, width=2), marker=dict(size=8)), secondary_y=True)
...
with tab4:
    ...
    with col_r:
        ...
        fig2.add_trace(go.Bar(name="TMO (min)", x=df_asesores["Asesor"], y=df_asesores["TMO (min)"],
                              marker_color=[RED if t>6 else AMBER if t>4.5 else GREEN
                                            for t in df_asesores["TMO (min)"]]))
    ...
    with col_l2:
        ...
        fig3 = go.Figure(go.Bar(x=df_objeciones["Frecuencia"], y=df_objeciones["Objeción"],
                                orientation="h",
    ...
    with col_r2:
        ...
        fig4 = go.Figure(go.Bar(x=df_objeciones["Resolución exitosa %"], y=df_objeciones["Objeción"],
                                orientation="h",
    ...
    st.warning(f"**Puntos críticos:** 'No tengo dinero' (2,840 casos, solo 38% resolución). "
               f"% Abandono promedio **{avg_abandono:.1f}%** — revisar apertura de llamada.")
    st.markdown("---")
...
with tab5:
    ...
    with col_r:
        ...
        fig2 = go.Figure(go.Bar(x=df_p["Impacto estimado $"], y=df_p["Palanca"],
    ...
    st.markdown("<br>", unsafe_allow_html=True)
    st.success(f"**Resumen ejecutivo:** Con las palancas propuestas se puede cerrar hasta **${total_impacto/1e6:.2f}M** adicional, "
               f"cubriendo el **{total_impacto/gap_jun*100:.0f}%** del gap. "
